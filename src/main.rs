...
fn main() {
    ...
    if params.txg_r1s.len() > 0 {
        ...
        eprintln!(
            "{:?}\t{:?}\t{:?}\t{:?}",
            params.txg_r1s, params.txg_trim_r1s, params.txg_r2s, params.txg_trim_r2s
        );
    ...
    if let Some(het_kmers) = &params.paired_kmers {
        ...
        loop {
            ...
            if tag == "HOM\t" {
                ...
                kmer_type.insert(
                    buf1[0..(bytes1 - 1)].to_vec(),
                    (kmer_id, KMER_TYPE::UNPAIRED_HET),
                );
                ...
                kmer_type.insert(
                    buf1[0..(bytes1 - 1)].to_vec(),
                    (kmer_id, KMER_TYPE::PAIRED_HET),
                );
                kmer_type.insert(
                    buf3[0..(bytes1 - 1)].to_vec(),
                    (kmer_id + 1, KMER_TYPE::PAIRED_HET),
                );
                kmers.insert(buf1[0..(bytes1 - 1)].to_vec(), kmer_id);
...
fn process_fasta(params: &Params, kmer_ids: &HashMap<Vec<u8>, i32>) {
    ...
    let mut linedex = 0;
    let writer =
        File::create(format!("{}/fasta_kmers.bin", params.output)).expect("Unable to create file");
...
fn process_longreads(params: &Params, kmer_ids: &HashMap<Vec<u8>, i32>) {
    ...
    for (filenum, _r) in to_iterate.iter() {
        f.write_all(
            format!("{}/molecules_longreads_{:03}.bin\n", params.output, filenum).as_bytes(),
        )
    ...
    to_iterate.par_iter().for_each(|(filenum, read_file)| {
        ...
        let writer = File::create(format!(
            "{}/molecules_longreads_{:03}.bin",
            params.output, filenum
        ))
...
fn process_hic(params: &Params, kmer_type: &HashMap<Vec<u8>, (i32, KMER_TYPE)>) {
    ...
    to_iterate
        .par_iter()
        .for_each(|(filenum, r1_file, r2_file)| {
...
fn process_txg(params: &Params, kmer_ids: &HashMap<Vec<u8>, i32>) {
    ...
    to_iterate
        .par_iter()
        .for_each(|(filenum, r1_file, r1_trim, r2_file, r2_trim)| {
            let mut r1_reader = parse_fastx_file(&r1_file).expect("invalid path/file");
            let mut r2_reader = parse_fastx_file(&r2_file).expect("invalid path/file");
            let mut result: Vec<u8> = Vec::new();
            let r1_trim = *r1_trim;
                    ...
                    if let Some(barcode_id) = barcodes.get(&r1_rec.sequence()[0..16]) {
                        ...
                        for (index, kmer, _) in r1_seq.canonical_kmers(params.kmer_size, &rc1) {
                            if index > r1_trim {
                                if let Some(kmer_id) = kmer_ids.get(kmer) {
                                    result
                                        .write_i32::<LittleEndian>(*barcode_id)
                                        .expect("buffer fill fail");
                                    result
                                        .write_i32::<LittleEndian>(*kmer_id)
                                        .expect("buffer fail");
                                    writer.write_all(&result).expect("write fail");
                                    result.clear();
                                }
...
fn load_params() -> Params {
    ...
    let txg_barcodes = match params.value_of("txg_barcodes") {
        ...
        None => {
            assert!(
                txg_r1s.len() == 0,
                "if you specify txg_files, you must also specify txg_barcodes"
            );
            None
        }
    };
